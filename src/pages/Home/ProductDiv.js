...
const ProductDiv = (props) => {
  ...
  return (<>
          <div onMouseEnter={()=>{sethover(true)}}  onClick={handleClick} onMouseLeave={()=>{sethover(false)}}  className='cursor-pointer productDiv rounded-[20px] w-[220px] pt-[5px] pr-[10px] pl-[17px] mx-auto h-[390px] '>
           <div key={props.index} >
              <div className={`flex ${props.item.image==White?('text-[#C94D3F]'):('text-[#78A962]')} pl-2 `}><img src={props.item.label} alt='' /> <p className='pl-1 text-[10px] leading-[21px] font-normal '>{props.item.label_2}</p></div>
