...
const arrayOne = [
  {
    ...
    image: Window,
     review: Star,
    title:<p className="text-[13px] font-normal leading-[19px]">EX DISPLAY : MSI Pro 16 <br/> Flex-036AU 15.6  MULTITOUCH <br/> All-In-On...</p> ,
    price: '$499.00' ,
...
const sideImageRef=useRef(null)
  return (
    ...
    <header className="">
        ...
        className=" relative flex lg:flex-row xsm:flex-col justify-between gap-[30px] items-center max-w-[1400px] mx-auto">
          <div className="flex">
            ...
            <button className="lg:pt-[4px] cursor-pointer">
              <motion.img whileTap={{scale:0.6}}  src={Button} alt="" onClick={handleDropDownClick} ref={imageRef}/>
            </button>
            {dropDown?(  
       <motion.div
       ref={dropDownRef}
        ...
        <div className="flex border-b-[#CACDD8] border-solid border-b-[1px] pl-2 pt-2 pr-3">
          ...
          <div>
            ...
            <div className="flex">
              ...
              <p className="text-[#000000] font-medium "> 9:00 AM - 6:00 PM</p>  </div>
          
            <div className="flex pb-4">
              <p className="text-[#A2A6B0]">Sat:</p>
              <p className="text-[#000000] font-medium "> 11:00 AM - 5:00 PM</p></div>
            
          </div>
        </div>
        <div className="flex pt-2 pb-2 border-b-[#CACDD8] border-solid border-b-[1px] pl-[5.5px]">
           <img src={location} alt="" className="mt-[-17px]"/>
            <p className="pl-2">Address: 1234 Street Adress, <br /> City Address, 1234</p>
           </div>
           <div className="pt-2 pb-3 pl-4">
            <p>Phones: <a href="" className="text-[#0156FF]">(00) 1234 5678</a> </p>
            <p>E-mail: <a href="" className="text-[#0156FF]">shop@email.com</a></p>
           </div>
           </motion.div>
          ...
          <div className="flex item-center">
            ...
            <div className="flex mt-[3px]">
              <a href="https://www.facebook.com/"><img src={Ficon} alt="" className="ml-[15px] cursor-pointer" /></a>
              <a href="https://www.instagram.com/"><img src={Iicon} alt="" className="ml-[6px] cursor-pointer" /></a>
            </div>
          </div>
        </motion.div>  
      </div>
      ...
      <Link to="/"><img src={Logo} alt="" className=""/></Link>
       <div className="xsm:block xl:hidden pr-[15px] pt-[3px]"> <motion.i onClick={()=>{setsideBar(!sideBar)}}  ref={sideImageRef} whileTap={{scale:0.6}} className=" fa-solid fa-bars text-[27px] text-[#0156FF]  "></motion.i></div>
       


          {search===true ? (  <div className="flex items-center justify-center">
          ...
          <div className="relative flex items-center">
            ...
            className="flex items-center 2xl:gap-[20px] gap-[13px] ">
              <li className={` cursor-pointer flex items-center justify-center  font-semibold rounded-[100px] w-[80px] h-[37px] ${hoveredItem_3?('bg-[#0156FF] text-[#ffff] '):('')}]`} onMouseEnter={()=>{setHoveredItem_3(true)}} onMouseLeave={()=>{ 
                const timeout = setTimeout(() => {
                  setHoveredItem_3(false);
                  }, 1000); 
               if (hoveredItem_3Ref.current) {
                hoveredItem_3Ref.current.addEventListener('mouseenter',()=>{
                  clearTimeout(timeout);
                })
               }
              }}>Laptops</li>
              <li className=" cursor-pointer  font-semibold">Desktop PCs</li>
              <li className=" cursor-pointer  font-semibold">Networking Devices</li>
              <li className=" cursor-pointer  font-semibold">Printers & Scanners</li>
              <li className=" cursor-pointer  font-semibold">PC Parts</li>
              <li className=" cursor-pointer  font-semibold">All Other Products</li>
              <li className=" cursor-pointer  font-semibold">Repairs</li>
            </motion.ul>
           <button className="text-[#0156FF] font-semibold border-2 border-solid border-[#0156FF]  pt-[8px] pb-[8px] pl-[28px] pr-[28px] rounded-3xl">Our Deals</button>
        
           </motion.div>
           )}
            {hoveredItem_3?(<div className="absolute z-20 top-[156px]" ref={hoveredItem_3Ref} onMouseLeave={()=>{setHoveredItem_3(false)}} >
    <div  className="flex h-[400px] bg-white  max-w-[1398px] w-full mx-auto border-[1px] border-[#CACDD8] border-solid">
              ...
              <div className="max-w-[330px] w-full border-[1px] border-[#CACDD8] border-solid">
                ...
                setmenu_3(false)}}
                  className={`cursor-pointer w-full flex justify-between items-center hover:bg-[#F5F7FF] pl-[28px] pt-[10px] pb-[13px] pr-[24px] 
                    ...
                    <li className=" list-none text-[14px] leading-[21px] font-[600]" >{item.title}</li>
                    {hoveredItem === index && <img src={item.img} alt="" />}
                  </div>
                ))}
              </div>
              {menu?(<div className="max-w-[263px] w-full border-[1px] border-[#CACDD8] border-solid">
              {secondMenu.map((item , index)=>(
                  <div onClick={()=>{setmenu_2(menu_2 => !menu_2)
                  setmenu_3(false)
                  ...
                  className={`cursor-pointer w-full flex justify-between items-center hover:bg-[#F5F7FF] pl-[28px] pt-[10px] pb-[13px] pr-[24px] 
                    ...
                    <li className=" list-none text-[14px] leading-[21px] font-[600]" >{item.title}</li>
                    {hoveredItem_2 === index && <img src={item.img} alt="" />}
                  </div>
                ))}
              </div>):('')}
              {menu_2?(<div className="max-w-[263px] w-full border-[1px] border-[#CACDD8] border-solid">
              {thirdMenu.map((item , index)=>(
                  <div 
                  key={index + '3menu'}
                  onClick={()=>{setmenu_3(menu_3 => !menu_3)}}
                  className={`w-full flex justify-center  pl-[28px] pt-[10px] pb-[13px] pr-[24px]`}>
                    <li  className=" cursor-pointer flex gap-[2px] list-none text-[14px] leading-[21px] font-[600]" >{item.title} {item.num}</li>
                  </div>
                ))}
              </div>):('')}
              {menu_3?(<div className="flex w-full justify-around border-[1px] border-[#CACDD8] border-solid">
           ...
           {images.map((item,index)=>(
            <div  key={index+'imags'}>  <img src={item.img} alt="" /></div>
           ))}</div>
           </div>):('')}
           </div>
      ...
      {search?(  <motion.img
        whileTap={{scale:0.6}} 
        src={Cross} alt="" className="mr-8 cursor-pointer" onClick={handleCrossClick}/>  ):(       <motion.img
          whileTap={{scale:0.6}} 
          src={Search} alt="" className=" mr-8 cursor-pointer" onClick={handleSearchClick}/>     )}
        <div className="relative flex flex-col items-center"><motion.img
        whileTap={{scale:0.6}} 
        ref={cartimageRef}
        src={Cart} alt="" className="mt-[10px] mr-8 cursor-pointer" onClick={()=>{setshoppingCart(!shoppingCart)}}/>
         {shoppingCart&&(<div ref={cartRef} className="absolute z-50  right-[22px] top-[22px] w-[311px]  bg-white pt-[23px] border-[#CACDD8] border-[1px] border-solid">
    <div className="relative"><div class="absolute right-[15px] top-[-26px] h-[15px] w-[15px]  border-solid border-[#ffffff] border-b-8 border-l-8 transform rotate-45"></div></div>
          <h1 className="text-center font-[600] text-[18px] leading-[26px]">My Cart</h1>
          <p className="font-[400] leading-[18px] text-[12px] text-[#000000] text-center pt-[6px]">2 item in cart</p>
          <div className="pt-[17px] w-full pb-[20px] flex justify-center border-b-[1px] border-solid border-[#CACDD8]">
           <Link to="/ShoppingCart"> <button className="text-[#0156FF] border-solid border-[#0156FF] border-2 rounded-[50px] h-[37px] w-[250px] text-[14px] leading-[21px] font-[600]">View or Edit Your Cart</button></Link>
          </div>
          {Array.from({length:count}).map((_,index)=>(
            <div key={index+'cart'} className="flex justify-between pt-[17px] pb-[19px] items-start pl-[15px] pr-[8px] border-b-[1px] border-solid border-[#CACDD8]" >
              <div className="flex items-center h-[70px]"><h1>{cart.count}</h1></div>

              <img src={cart.img} alt="" className="w-[65px] h-[65px]"/>
              <p>{cart.title}</p>
              <div className="flex flex-col gap-[5px]">
              <div><img src={cart.cross} alt="" className="max-w-[20px]  h-[20px]"/></div>
              <div><img src={cart.edit} alt="" className="max-w-[20px]  h-[20px]"/></div>
              </div>
            </div>
          ))}
          <h1 className="flex items-center justify-center text-[14px] leading-[18px] font-[600] text-[#A2A6B0] pt-[16px]">Subtotal:<p className=" text-black pl-2 text-[18px] leading-[26px] font-[600]">$499.00</p></h1>
        <div className="flex items-center justify-center flex-col gap-[10px] pt-[13px] pb-[24px]">
         <Link to="/Checkout"> <button  className="w-[250px] h-[37px] bg-[#0156FF] rounded-[50px] text-[14px] leading-[21px] font-[600] text-[#ffff]">Go to Checkout</button></Link>
          <button className="w-[250px] h-[37px] bg-[#FFB800] rounded-[50px] text-[14px] leading-[18px] font-[600] flex items-center justify-center gap-1 "><p>Check out with </p><img src={pay} alt="" /></button>
        </div>
        </div>)}
        </div>
        <motion.img
        ref={image}
        whileTap={{scale:0.6}} 
        src={Avatar} alt="" onClick={()=>{setdropDown_2(!dropDown_2)}} className=" cursor-pointer"/> 
         <div className=" absolute right-[57px] rounded-full w-[16px] h-[17px] bg-[#0156FF] flex items-center justify-center text-[10px] leading-[15px]  text-[#ffff]">2</div>
         {dropDown_2==1?(<div className="relitive " ref={ drop}>
        ...
        <div className="relative"><div class="absolute right-[1px] top-[-26px] h-0 w-0 mt-[-4px] ml-[18px] border-solid border-[white] border-b-8 border-l-8 transform rotate-45"></div></div>
          {lies.map((item , index)=>(
          <Link to={item.path} key={index+'lies'}  className="block hover:font-[600] list-none cursor-pointer text-[14px] leading-[28px] font-medium">{item.title}</Link>
          ))}
        </div>
         </div>):('')}
         </div  >
  ...
  {sideBar==false? (<div className="relative w-full "><motion.input
              type="search"
...
{sale.map((item , index)=>
<Link to={item.path} key={index+'sale'} className={`list-none max-w-[200px] w-full  hover:text-[black] hover:font-[700]`}>{item.title}</Link>
)}
  </motion.div>)}
  </div>
